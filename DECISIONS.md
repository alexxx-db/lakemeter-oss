...
**Alternatives considered.** Read pricing directly from Unity Catalog via SQL warehouses: rejected because it adds a warehouse dependency and cold-start latency to every calculation, and breaks the "app must not depend on a running warehouse" constraint.
...
**Alternatives considered.** Alembic: rejected because of operational burden for admins, and Lakebase connections use short-lived OAuth tokens that complicate offline migration runners.
