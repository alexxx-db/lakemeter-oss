...
def _iter_api_routes(routes, prefix=""):
    """Flatten FastAPI routes through `_IncludedRouter` wrappers (FastAPI ≥0.140)."""
    for route in routes:
...
class TestUsersAuthZ:
    def test_user_and_chat_routes_require_get_current_user(self):
        ...
        for path, route in _iter_api_routes(app.routes):
            if not (path.startswith("/api/v1/users") or path.startswith("/api/v1/chat")):
                continue
