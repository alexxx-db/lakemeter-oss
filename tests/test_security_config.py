...
def _iter_routes(routes, prefix=""):
    """Flatten FastAPI/Starlette routes, including `_IncludedRouter` wrappers."""
    for route in routes:
...
def _route_paths(monkeypatch, **env):
    ...
    for key, value in env.items():
        monkeypatch.setenv(key, value)
    import app.main
    return list(_iter_routes(app.main.app.routes))
